...
export class BookingsService {
  ...
  async create(dto: CreateBookingDto) {
    ...
    this.db
      ...
      .then(([users]) => {
        if (users.length > 0) {
          const user = users[0];
          this.mailService.sendBookingCreatedMail(
            user.email,
            user.full_name,
            shortId,
            new Date(dto.checkInDate),
            (newBooking as any).grand_total || (newBooking as any).grantTotal,
          );
        }
      })
      .catch((err) =>
        this.logger.error('Failed to fetch user for email sending', err),
      );
    ...
    try {
      ...
      this.db
        ...
        .then(([users]) => {
          if (users.length > 0) {
            const user = users[0];
            this.mailService.sendBookingConfirmedMail(
              user.email,
              user.full_name,
              booking.short_id,
        ...
        .catch((err) =>
          this.logger.error('Failed to fetch user for confirm email', err),
        );
  ...
  async createCoupon(createCouponDto: CreateCouponDto) {
    const [existCp] = await this.db.query<RowDataPacket[][]>(
      'SELECT * FROM coupons WHERE code = ? AND coupon_status = ?',
      [createCouponDto.code, 'Active'],
    );
  ...
  async findOneCoupon(id: string) {
    const [rows] = await this.db.query<RowDataPacket[][]>(
      'SELECT * FROM coupons WHERE id = ? AND coupon_status = ?',
      [id, 'Active'],
    );
    return rows[0];
  }
  ...
  async applyCoupon(applyCouponDto: ApplyCouponDto) {
    ...
    const [rows] = await this.db.query<RowDataPacket[][]>(
      'CALL sp_use_coupon(?,?,?)',
      [bookingId, couponCode, discountAmount],
    );
    return rows[0];
  }
  ...
  async deleteCoupon(id: string) {
    const [rows] = await this.db.query<RowDataPacket[][]>(
      'UPDATE coupons SET coupon_status = "Inactive", expired_at = NOW() WHERE id = ?',
      [id],
    );
    return rows[0];
  }
