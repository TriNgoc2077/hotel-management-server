...
export class UsersService implements OnModuleInit {
  ...
  async onModuleInit() {
    try {
      const [rows] = await this.pool.query<RowDataPacket[]>(
        'SELECT token, expired_at FROM blacklisted_tokens WHERE expired_at > ?',
        [Date.now()],
      );
      ...
      Logger.error(
        'Failed to load blacklisted tokens, perhaps migration not run:',
        e.message,
      );
    }
  ...
  private cleanupBlacklistedTokens() {
    ...
    this.pool
      .query('DELETE FROM blacklisted_tokens WHERE expired_at <= ?', [now])
      .catch((e) => console.error('Cleanup DB error:', e.message));
  }
  ...
  async create(createUserDto: CreateUserDto) {
    ...
    try {
      await this.pool.query('CALL sp_create_user(?, ?, ?, ?, ?, ?, ?, ?)', [
        id,
  ...
  async findByRefreshToken(refreshToken: string, agent: string) {
    const [rows] = await this.pool.query<RowDataPacket[]>(
      'SELECT * FROM tokens WHERE refresh_token = ? AND agent = ?',
      [refreshToken, agent],
    );

    if (rows.length === 0) {
      throw new NotFoundException(
        `User with refresh token ${refreshToken} not found or expired`,
      );
    }
  ...
  async update(id: string, updateUserDto: UpdateUserDto) {
    try {
      ...
      await this.pool.query('CALL sp_update_user(?, ?, ?, ?, ?, ?, ?)', [
        id,
  ...
  async updateRefreshToken(
    ...
    await this.pool.query('CALL sp_update_refresh_token(?, ?, ?, ?)', [
      userId,
      refreshToken,
      expiredAt,
      agent,
  ...
  async updateFcmToken(userId: string, fcmToken: string, agent: string) {
    await this.pool.query('CALL sp_update_fcm_token(?, ?, ?)', [
      userId,
      fcmToken,
      agent,
    ]);
  ...
  async removeToken(userId: string, agent: string) {
    await this.pool.query('CALL sp_remove_token(?, ?)', [userId, agent]);
    return true;
  }
  ...
  async blacklistToken(token: string, expiredAt: number) {
    ...
    try {
      await this.pool.query('CALL sp_blacklist_token(?, ?)', [
        token,
        expiredAt,
      ]);
    } catch (error) {
